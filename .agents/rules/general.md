...
## Strict File Editing Rules (CRITICAL)
- **Modifying existing files MUST use `replace_in_file` — NEVER use `write_to_file` to overwrite an existing file.** `write_to_file` is only allowed for creating new files.
- **Multiple independent SEARCH/REPLACE blocks can be combined in a single `replace_in_file` call** for parallel execution. Use multiple `------- SEARCH` / `=======` / `+++++++ REPLACE` blocks within one `diff` parameter when the changes target the same file at unrelated locations.
- If a SEARCH block fails to match, re-read the file and try again with the exact content — the file may have been modified by biome/formatting since last read.
