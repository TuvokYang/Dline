...
import type { ClineRecommendedModel } from "@shared/proto/dline/models"
import type { OnboardingModel, OnboardingModelGroup } from "@shared/proto/dline/state"
...
function toOnboardingModel(
	...
	return {
		...
		info: catalogInfo
			? {
					contextWindow: catalogInfo.capabilities?.contextWindow ?? 0,
					supportsImages: catalogInfo.capabilities?.supportsImages ?? false,
					supportsPromptCache: catalogInfo.capabilities?.supportsPromptCache ?? false,
					inputPrice: catalogInfo.pricing?.inputPrice ?? 0,
					outputPrice: catalogInfo.pricing?.outputPrice ?? 0,
					tiers: catalogInfo.pricing?.tiers ?? [],
				}
			: undefined,
	}
}
