...
import type { OnboardingModel, OnboardingModelGroup, OpenRouterModelInfo } from "@shared/proto/index.dline"
						...
						{searchTerm &&
							searchedModels.map(([id, info]) => {
								...
								const modelInfo: OpenRouterModelInfo = {
									...
									thinkingConfig: info.capabilities?.thinking ?? undefined,
		...
		async (updateModelId: boolean, step: number) => {
			...
			if (modelSelected) {
				await handleFieldsChange({
					planModeProfile: selectedModelId,
					actModeProfile: selectedModelId,
				})
			}
			hideAccount()
			hideSettings()
