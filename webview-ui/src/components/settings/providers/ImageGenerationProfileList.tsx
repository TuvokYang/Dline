	...
	return (
		...
		<div className="flex items-center justify-between">
			<div>
				...
				<div className="text-xs text-description">Independent image endpoints used by API Profile Image bindings.</div>
			</div>
			<button className="inline-flex min-h-7 items-center gap-1 rounded-xs border px-2 text-xs" onClick={onAdd} type="button">
		...
		{profiles.length === 0 ? <div className="text-xs text-description">No independent image profiles configured.</div> : null}
		</div>
	)
}
