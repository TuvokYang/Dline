...
import { type ClineRecommendedModel, ClineRecommendedModelsResponse } from "@shared/proto/dline/models"
...
const ClineModelPicker: React.FC<ClineModelPickerProps> = ({ isPopup, showProviderRouting, initialTab }) => {
	const { handleModeFieldsChange, handleFieldChange } = useApiConfigurationHandlers()
	const { apiConfiguration, favoritedModelIds, clineModels, refreshClineModels, mode: currentMode } = useExtensionState()
	...
	const handleModelChange = (newModelId: string) => {
		...
		handleModeFieldsChange(
			{
				clineModelId: { plan: "planModeProfile", act: "actModeProfile" },
				clineModelInfo: { plan: "planModeProfile", act: "actModeProfile" },
			},
			{
	...
	const showBudgetSlider = useMemo(() => {
		...
		return (
			Object.entries(clineModels ?? {})?.some(([id, m]) => id === selectedModelId && m.capabilities?.thinking) ||
		...
		<div style={{ width: "100%", paddingBottom: 2 }}>
			...
			{hasInfo ? (
				...
				<p
					...
					The extension automatically fetches the latest Dline model list. If you're unsure which model to choose, Dline
