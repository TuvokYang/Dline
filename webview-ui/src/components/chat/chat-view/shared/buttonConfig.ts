...
export function getButtonConfig(message: ClineMessage | undefined, _mode: Mode = "act"): ButtonConfig {
	...
	if (message.type === "ask") {
		switch (message.ask) {
			...
			case "qna_respond":
				return BUTTON_CONFIGS.qna_respond
			case "generate_report":
				return BUTTON_CONFIGS.generate_report
			case "status_acknowledgment":
				return BUTTON_CONFIGS.status_acknowledgment
			...
			case "focus_chain_change": {
				// If plan has prefix markers ([+] or [-]), already approved — no buttons
				try {
