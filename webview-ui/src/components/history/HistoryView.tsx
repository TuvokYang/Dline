import { BooleanRequest, EmptyRequest, StringArrayRequest } from "@shared/proto/dline/common"
import { GetTaskHistoryRequest, TaskFavoriteRequest } from "@shared/proto/dline/task"
			...
			TaskServiceClient.deleteTasksWithIds(StringArrayRequest.create({ value: [id] }))
				.then((result: any) => {
					...
					if (result.skippedLocked > 0) {
						alert("This task is currently in use by another instance and cannot be deleted.")
					}
				})
			...
			if (ids.length > 0) {
				TaskServiceClient.deleteTasksWithIds(StringArrayRequest.create({ value: ids }))
					.then((result: any) => {
						...
						if (result.skippedLocked > 0) {
							alert(
								`${result.skippedLocked} task(s) are currently in use by another instance and were not deleted.`,
							)
						}
					})
