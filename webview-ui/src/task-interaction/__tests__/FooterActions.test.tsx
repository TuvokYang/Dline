...
describe("FooterActions", () => {
	...
	it("dispatches a task-level Resume fallback through the task command boundary", async () => {
		const view = approvalView()
		delete view.activeInteraction
		view.phase = "paused"
		view.footer.actions = [
		...
		await waitFor(() => expect(dispatchTaskAction).toHaveBeenCalledWith("resume"))
	})

	it("disables a task-level action when its dispatcher is unavailable", () => {
		const view = approvalView()
		delete view.activeInteraction
		view.phase = "paused"
		view.footer.actions = [
		...
		render(<FooterActions dispatch={dispatch} draft={{ text: "", images: [], files: [] }} view={view} />)
