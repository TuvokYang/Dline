import { DLINE_ENVIRONMENT_ENV, DLINE_ENVIRONMENTS } from "@cline/shared";
...
describe("cline builtin spec defaults.baseUrl", () => {
	const originalEnvironment = process.env[DLINE_ENVIRONMENT_ENV];

	beforeEach(() => {
		delete process.env[DLINE_ENVIRONMENT_ENV];
	});
	...
	afterEach(() => {
		if (originalEnvironment === undefined) {
			...
			process.env[DLINE_ENVIRONMENT_ENV] = originalEnvironment;
		}
	});

	it("re-resolves baseUrl when DLINE_ENVIRONMENT changes between reads", () => {
		const spec = findClineSpec();

		expect(spec.defaults?.baseUrl).toBe(
			`${DLINE_ENVIRONMENTS.production.apiBaseUrl}/api/v1`,
		);

		process.env[DLINE_ENVIRONMENT_ENV] = "staging";
		expect(spec.defaults?.baseUrl).toBe(
			`${DLINE_ENVIRONMENTS.staging.apiBaseUrl}/api/v1`,
		);

		process.env[DLINE_ENVIRONMENT_ENV] = "local";
		expect(spec.defaults?.baseUrl).toBe(
			`${DLINE_ENVIRONMENTS.local.apiBaseUrl}/api/v1`,
		);
