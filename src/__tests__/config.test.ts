...
// Mock os.homedir() to point to tempDir so that user config path
// (~/.dline/endpoints.json) resolves to the test directory
...
describe("ClineEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		// Create .cline directory for user config path
		await fs.mkdir(path.join(tempDir, ".dline"), { recursive: true })

		// Mock os.homedir() to return tempDir so user config path resolves correctly
	...
	describe("valid config parsing", () => {
		...
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".dline", "endpoints.json"), JSON.stringify(validConfig), "utf8")
		...
		it("should work without endpoints.json (standard mode)", async () => {
			...
			config.appBaseUrl.should.equal("https://app.dline.bot")
			config.apiBaseUrl.should.equal("https://api.dline.bot")
		})
	...
	describe("invalid JSON handling", () => {
		it("should throw ClineConfigurationError for invalid JSON syntax", async () => {
			await fs.writeFile(path.join(tempDir, ".dline", "endpoints.json"), "{ invalid json }", "utf8")
		...
		it("should throw ClineConfigurationError for truncated JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".dline", "endpoints.json"), '{"appBaseUrl": "https://test.com"', "utf8")
		...
		it("should throw ClineConfigurationError for empty file", async () => {
			await fs.writeFile(path.join(tempDir, ".dline", "endpoints.json"), "", "utf8")
		...
		it("should throw ClineConfigurationError for non-object JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".dline", "endpoints.json"), '"just a string"', "utf8")
		...
		it("should throw ClineConfigurationError for null JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".dline", "endpoints.json"), "null", "utf8")
	...
	describe("missing required fields", () => {
		it("should throw ClineConfigurationError when appBaseUrl is missing", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".dline", "endpoints.json"), JSON.stringify(config), "utf8")
	...
	describe("self-hosted mode behavior", () => {
		...
		it("should use custom endpoints from file", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".dline", "endpoints.json"), JSON.stringify(customConfig), "utf8")
	...
	describe("initialization behavior", () => {
		...
		it("should return undefined when accessing config before initialization", async () => {
			// Already reset in beforeEach, config should be undefined
			const config = ClineEndpoint.config
			;(config === undefined).should.be.true()
		})
	})
	...
	describe("bundled endpoints.json behavior", () => {
		...
		beforeEach(async () => {
			...
			// Point os.homedir() to bundledDir so user config path is isolated
			mockHomedir.mockReturnValue(bundledDir)

			// Reset ClineEndpoint singleton state for clean bundled tests
			;(ClineEndpoint as any)._instance = null
			;(ClineEndpoint as any)._initialized = false
			;(ClineEndpoint as any)._extensionFsPath = undefined
		})
		...
		it("should prefer bundled endpoints.json over user file", async () => {
			...
			// Set up both configs (mockHomedir points to bundledDir)
			await fs.writeFile(path.join(bundledDir, "endpoints.json"), JSON.stringify(bundledConfig), "utf8")
			await fs.mkdir(path.join(bundledDir, ".dline"), { recursive: true })
			await fs.writeFile(path.join(bundledDir, ".dline", "endpoints.json"), JSON.stringify(userConfig), "utf8")
		...
		it("should fall back to user endpoints.json when bundled is not present", async () => {
			...
			// Only create user config, no bundled config (mockHomedir points to bundledDir)
			await fs.mkdir(path.join(bundledDir, ".dline"), { recursive: true })
			await fs.writeFile(path.join(bundledDir, ".dline", "endpoints.json"), JSON.stringify(userConfig), "utf8")
