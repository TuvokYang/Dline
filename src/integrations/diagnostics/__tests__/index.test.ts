import { DiagnosticSeverity, FileDiagnostics } from "@shared/proto/index.dline"
...
/** Normalize path separators in file paths within expected strings to match the current platform. */
function _norm(p: string): string {
	// Only replace / that appear in file paths (between word characters), not in \n or other escape sequences
	return p.replace(/([a-zA-Z_.-])\/([a-zA-Z_.-])/g, `$1${path.sep}$2`)
}
...
describe("Diagnostics Tests", () => {
	...
	describe("diagnosticsToProblemsString", () => {
		let _getCwdStub: any /* sinon.SinonStub → vitest */

		beforeEach(() => {
			_getCwdStub = vi.spyOn(pathUtils, "getCwd").mockResolvedValue("/workspace")
		})
