...
import { DiagnosticSeverity, FileDiagnostics } from "@/shared/proto/dline"
...
export abstract class DiffViewProvider {
	 ...
	 *
	 * ⚠️ CRITICAL: editType MUST be set BEFORE calling open().
	 *   "modify" → reads file content into originalContent.
	 *   "create" → initializes originalContent as empty.
	 *   undefined → WILL OVERWRITE file as empty via fs.writeFile(path, "").
	 * Always ensure editType is properly set to "modify" or "create" before calling.
	 */
	...
	async saveChanges(): Promise<{
		...
		if (!saved) {
			throw new Error(`Failed to save document: ${this.absolutePath}`)
		}
		...
		const wroteLines = normalizedNewContent.split("\n").length
		const savedLines = normalizedPostSaveContent.split("\n").length
		const formatterChanged = normalizedPreSaveContent !== normalizedPostSaveContent
	...
	async reset() {
		Logger.debug(
			`DiffViewProvider.reset: isEditing=${this.isEditing} absolutePath=${this.absolutePath} editType=${this.editType}`,
		)
		this.isEditing = false
