...
describe("notifications", () => {
	...
	it("escapes quotes and backslashes for macOS notifications", async () => {
		...
		const platformStub = vi.fn().mockReturnValue("darwin")
		...
		expect(execaStub)
		expect(execaStub.mock.calls[0][0]).to.equal("osascript")
		const script = (execaStub.mock.calls[0][1] as string[])[1]
	...
	it("passes title and combined subtitle/message to notify-send on Linux", async () => {
		...
		expect(execaStub)
		expect(execaStub.mock.calls[0][0]).to.equal("notify-send")
		expect(execaStub.mock.calls[0][1]).to.deep.equal(["Cline", "Approval Required\ntest"])
	})
	...
	it("encodes Windows PowerShell notifications before dispatch", async () => {
		...
		expect(execaStub)
		expect(platformStub)
		expect(execaStub.mock.calls[0][0]).to.equal("powershell")
