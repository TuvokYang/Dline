...
export class StandaloneTerminalProcess extends EventEmitter<TerminalProcessEvents> implements ITerminalProcess {
	...
	/**
	 * Decode a Buffer to string using the system terminal encoding.
	 *
	 * Uses the encoding detected via chcp (Windows) or locale (Unix) directly,
	 * with a safe fallback to UTF-8 on decode failure.
	 *
	 * @param data The Buffer to decode
	 * @returns Decoded string
	 */
	private decodeBuffer(data: Buffer): string {
		// Use system encoding if available (detected via chcp/locale)
		if (this.systemEncoding) {
			try {
				return iconv.decode(data, this.systemEncoding)
			} catch {
				// iconv decode failed (e.g., unsupported encoding name), fallback to UTF-8
			}
		}
		// Fallback: no system encoding or decode failed
	...
	private getShellArgs(shell: string, command: string): string[] {
		if (process.platform === "win32") {
			...
			// Switch to UTF-8 code page (65001) before executing command on cmd
			return ["/c", `chcp 65001 > nul && ${command}`]
		}
