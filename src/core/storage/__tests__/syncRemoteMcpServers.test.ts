...
describe("syncRemoteMcpServersToSettings", () => {
	...
	describe("edge cases", () => {
		...
		it("should set McpHub isUpdatingFromRemoteConfig flag during write", async () => {
			...
			const mockMcpHub = {
				setIsUpdatingFromRemoteConfig: vi.fn(),
			}

			await syncRemoteMcpServersToSettings([{ name: "test", url: "https://test.com" }], tempDir, mockMcpHub as any)

			mockMcpHub.setIsUpdatingFromRemoteConfig.mock.calls.some((c: unknown[]) => c[0] === true).should.be.true()
			mockMcpHub.setIsUpdatingFromRemoteConfig.mock.calls.some((c: unknown[]) => c[0] === false).should.be.true()

			const calls = mockMcpHub.setIsUpdatingFromRemoteConfig.mock.calls
			calls[0][0].should.equal(true)
			calls[1][0].should.equal(false)
		})
	})
})
