 ...
 *
 * Usage:
 * - Run tests normally: `npm run test`
 *   Tests will fail if generated prompts don't match existing snapshots
 *
 * - Update snapshots: `npm run test:snapshot`
...
const modelTestCases = [
	...
	{ family: ModelFamily.NATIVE_NEXT_GEN, modelId: "deepseek-v4-pro", providerId: "deepseek" },
...
describe("Prompt System Integration Tests", () => {
	...
	describe("Snapshot Testing", () => {
		for (const { family, modelId, providerId } of modelTestCases) {
			describe(`${family} Model Group`, () => {
				...
				it(`should generate consistent native tools object when enabled`, async (ctx) => {
					...
					await runPromptTest(ctx, context, modelId, async ({ tools }) => {
				...
				for (const { name: contextName, override } of contextVariations) {
					it(`should generate consistent prompt for ${providerId}/${modelId} with ${contextName} context`, async (ctx) => {
						const context: SystemPromptContext = {
							...baseContext,
							...override,
							providerInfo: makeProviderInfo(modelId, providerId),
						...
						await runPromptTest(ctx, context, modelId, async ({ systemPrompt, tools }) => {
							if (enableNativeToolCalls) {
								expect(tools).to.be.an("array").that.is.not.empty
								const toolNames = (tools as any[]).map((tool) => {
									if (tool?.type === "function") {
		...
		describe("Gemini 3 Specific", () => {
			for (const { family, modelId, providerId } of gemini3ModelTestCases) {
				const enableNativeToolCalls = isNativeToolsFamily(family)
				it(`should include parallel tool-calling guidance for ${providerId}/${modelId} when enabled`, async (ctx) => {
					const context: SystemPromptContext = {
						...baseContext,
						providerInfo: makeProviderInfo(modelId, providerId),
						enableNativeToolCalls,
					...
					await runPromptTest(ctx, context, modelId, async ({ systemPrompt }) => {
	...
	describe("Context-Specific Features", () => {
		...
		for (const { name, context, check } of featureTests) {
			it(`should include ${name}`, async (ctx) => {
				await runPromptTest(ctx, { ...baseContext, ...context }, "default", async ({ systemPrompt }) => {
	...
	describe("Error Handling", () => {
		it("should handle completely invalid context gracefully", async (_ctx) => {
			// mocha this.timeout removed — vitest uses testTimeout config: (TEST_TIMEOUT)
			const { systemPrompt } = await getSystemPrompt({} as SystemPromptContext)
			expect(systemPrompt).to.be.a("string")
		})

		it("should handle undefined context properties", async (_ctx) => {
			// mocha this.timeout removed — vitest uses testTimeout config: (TEST_TIMEOUT)
