...
export class PromptBuilder {
	...
	private static getEnabledTools(variant: PromptVariant, context: SystemPromptContext): ClineToolSpec[] {
		const tools = ClineToolSet.getEnabledToolSpecs(variant, context)
		// Filter out tools excluded by hierarchical tool gating (e.g., spawn_task from child tasks)
		if (context.disableTools && context.disableTools.length > 0) {
			return tools.filter((tool) => !context.disableTools?.includes(tool.id))
		}
		return tools
	}
