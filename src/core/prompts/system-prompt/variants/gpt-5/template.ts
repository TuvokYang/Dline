...
- In ACT MODE, focus on executing the plan. Errors are expected — analyze, fix, and retry (up to 7 times). Only call ask_followup_question when truly blocked. Do NOT call attempt_completion while focus chain items remain \`[ ]\`. Do NOT ask the user to switch modes.{{BROWSER_WAIT_RULES}}${hasEnabledMcpServers(context) ? "\n- MCP operations should be used one at a time, similar to other tool usage. Wait for confirmation of success before proceeding with additional operations." : ""}
