...
IMPORTANT NOTE: This tool CANNOT be used until you've confirmed from the user that any previous tool uses were successful and all tasks have been completed in full. Failure to do so will result in code corruption and system failure. Before using this tool, you must ask yourself in <thinking></thinking> tags if you've confirmed from the user that any previous tool uses were successful and all goals defined by the user have been completed. If not, then DO NOT use this tool.`,
	nativeDescription:
		"[TURN-END] Once you've completed the user's task, use this tool to present the final result to the user, including a brief and very short (1-2 paragraph) summary of the task and what was done to resolve it. This tool is ONLY for completing implementation or development tasks that have a task_progress checklist. You MUST verify ALL items in the task_progress list are marked [x] before calling this tool — if any [ ] remain, you CANNOT use attempt_completion. Do NOT use this tool to answer questions or chat — use qna_respond for Q&A interactions. You should only call this tool when you have completed all tasks in the task_progress list, and completed all changes that are necessary to satisfy the user's request. You should not provide the contents of the task_progress list in the result parameter, it must be included in the task_progress parameter.",
	...
	nativeResultInstruction: "A clear, brief and very short (1-2 paragraph) summary of the final result of the task.",
	commandInstruction:
