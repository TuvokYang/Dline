...
IMPORTANT NOTE: This tool CANNOT be used until you've confirmed from the user that any previous tool uses were successful and all tasks have been completed in full. Failure to do so will result in code corruption and system failure. Before using this tool, you must ask yourself in <thinking></thinking> tags if you've confirmed from the user that any previous tool uses were successful and all goals defined by the user have been completed. If not, then DO NOT use this tool.`,
	nativeDescription: `[TURN-END] After each tool use, the user will respond with the result of that tool use, including whether it succeeded or failed and any reason for failure. Once you have received those results, confirmed that all goals defined by the user are complete, and verified every item in the task_progress checklist is marked [x], use this tool to present the final result. Optionally provide an actionable CLI command that lets the user review the result.

This tool is only for completing implementation or development tasks that have a task_progress checklist. Do not use it to answer questions or chat; use qna_respond for Q&A. Do not put the checklist in the result text; provide it through the task_progress parameter.

IMPORTANT NOTE: This tool cannot be used until previous tool results have been confirmed and all tasks have been completed in full. If any checklist item remains incomplete, do not call attempt_completion.`,
