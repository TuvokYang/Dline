...
const prompts: Record<string, string> = {
	...
	editingFiles: `FILE EDITING RULES
...
- Implement stepwise; one tool per message. When all prior steps are user-confirmed successful, use attempt_completion.`,
	capabilities: `CURIOSITY & FIRST CONTACT
...
Include options/trade-offs when helpful, ask if plan matches, then add the exact mode-switch line.{subagentsSection}`,
	toolsNative: `TOOLS

You have access to a set of tools that you are expected to use to resolve the task.{subagentsSection}`,
	subagents: `
