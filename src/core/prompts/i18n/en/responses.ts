...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`,
	...
	replaceInFileMissingDiffError: `Failed to edit '{relPath}': The 'diff' parameter was empty.
...
Add the separator line between the SEARCH content and the REPLACE content.`,
	...
	diffSearchNotFound: `SEARCH content ({lineCount} lines) was not found in the file.
Diagnose:
...
Close the current block with +++++++ REPLACE before starting a new block.`,
	...
	diffDelimiterConflict: `Delimiter conflict: {blockType} content contains a line with {count} '{char}' characters matching the delimiter.
Use a different delimiter count (e.g. 8 or 9) to avoid this conflict. Example:
-------- SEARCH
========
++++++++ REPLACE`,

	diffDelimiterMismatch: `Delimiter count mismatch: SEARCH marker used {searchN} characters but close marker used {closeN} characters.
...
Ensure every REPLACE section ends with +++++++ REPLACE.`,
	...
	diffBlockOverlap: `Block #{blockIndex} overlaps with block #{prevIndex}.
...
Check that each SEARCH block references content after the previous replacement.`,
	...
	diffBlockOutOfOrder: `Block #{blockIndex} is out of file position order.
...
Re-read the file and verify the exact content of each SEARCH block.`,
	...
	writeToFileCriticalFail: `CRITICAL: You have failed to write this file {consecutiveFailures} times in a row. You MUST change your approach — do NOT retry write_to_file for this file again.
...
Each replace_in_file call should add no more than 50-100 lines of content at a time.`,
	...
	writeToFileSecondFail: `This is your {consecutiveFailures}{ordinalSuffix} failed attempt. The file content is likely too large to generate in one response. You must use a different strategy:
...
{toolReminder}`,
	...
	fileEditUpdatedContent:
		"The updated content has been successfully saved to {relPath}. (wrote {wroteLines} lines, saved {savedLines} lines)\n\n",

	fileEditSuccessContent:
		"The content was successfully saved to {relPath}. (wrote {wroteLines} lines, saved {savedLines} lines)\n",

	replaceEditSuccessContent:
		"The content was successfully replaced in {relPath}. (deleted {deletedLines} lines, added {addedLines} lines, saved {savedLines} lines)\n",
	...
	fileEditNotesWithoutChanges: "{newProblemsMessage}",
...
Failure to re-read before editing will result in replace_in_file edit errors, requiring subsequent attempts and wasting tokens. You DO NOT need to re-read these files after subsequent edits, unless instructed to do so.
	...
	clineIgnoreInstructions:
		"# .clineignore\n\n(The following is provided by a root-level .clineignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a {lockSymbol} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n{content}\n.clineignore",
	...
	clineRulesLocalDirInstructions:
		"# Local User Rules (.dline/rules/)\n\nThe following is provided by local user rules in {workspaceName} where the user has specified instructions:\n\n{content}",
