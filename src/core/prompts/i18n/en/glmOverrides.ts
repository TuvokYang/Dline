...
Use standard Markdown: - [ ] incomplete, - [x] complete. task_progress must be a parameter, not a standalone tool call.
...
`,
	toolUseBase: `Begin every task by exploring the codebase (e.g., list_files, search_files, read_file) and outlining the required changes. Do not implement until exploration yields enough context to state objectives, approach, affected files, and risks. Briefly summarize the plan, then proceed with implementation.
