...
export class LiteLlmHandler implements ApiHandler {
	...
	private get config() {
		return this.ctx.profile.litellm
	}
	private get apiKey() {
		...
		if (!this.client) {
			...
			try {
				this.client = createOpenAIClient({
					baseURL: this.baseUrl || "http://localhost:4000",
	...
	private async fetchLiteLlmModelInfo(publicModelName: string): Promise<LiteLlmModelInfoResponse["data"][number] | undefined> {
		const fetchedInfo = await this.fetchModelsInfo()

		if (!fetchedInfo?.data) {
			return undefined
		}

		return fetchedInfo.data.find((model) => model.model_name === publicModelName)
	}
	...
	private async fetchModelsInfo(): Promise<LiteLlmModelInfoResponse | undefined> {
		...
		const data = await fetchLiteLlmModelsInfo(client.baseURL, this.apiKey || "")
	...
	private async getModelCostInfo(publicModelName: string): Promise<{
		...
		try {
			const matchingModel = await this.fetchLiteLlmModelInfo(publicModelName)
	...
	async calculateCost(
		...
		const publicModelId = this.modelId || liteLlmDefaultModelId
