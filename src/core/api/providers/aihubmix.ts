...
export class AIhubmixHandler implements ApiHandler {
	...
	private get config() {
		return this.ctx.profile.aihubmix
	}
	private get apiKey() {
	...
	private get appCode() {
		return this.config?.aihubmixAppCode ?? "KUWF9311"
	}
	...
	private async *createAnthropicMessage(systemPrompt: string, messages: any[]): ApiStream {
		...
		const stream = await client.messages.create({
			model: modelId,
			temperature: 0,
			max_tokens: this.modelInfo?.capabilities?.maxTokens || 8192,
