...
export class AwsBedrockHandler implements ApiHandler {
	...
	private get config() {
		return this.ctx.profile.bedrock
	}
	private get apiKey() {
	...
	async *createMessage(systemPrompt: string, messages: ClineStorageMessage[], tools?: ClineTool[]): ApiStream {
		...
		const baseModelId =
			(this.config?.awsBedrockCustomSelected ? this.config?.awsBedrockCustomModelBaseId : modelId) || modelId
	...
	getModel(): { id: string; info: ModelInfo } {
		...
		const customSelected = this.config?.awsBedrockCustomSelected
		const baseModel = this.config?.awsBedrockCustomModelBaseId

		// Handle custom models
		if (customSelected && modelId) {
			// If base model is provided and valid, use its capabilities
			if (baseModel && bedrockModels[baseModel]) {
				return {
					id: modelId,
	...
	private async getAwsCredentials(): Promise<{
		...
		const useProfile =
			this.config?.awsAuthentication === "profile" || (!this.config?.awsAuthentication && !!this.config?.awsProfile)
		if (useProfile) {
			...
			if (this.config?.awsProfile) {
				providerOptions.profile = this.config?.awsProfile
			}
		}
		...
		return await AwsBedrockHandler.withTempEnv(
			() => {
				AwsBedrockHandler.setEnv("AWS_REGION", this.config?.awsRegion)
				if (useProfile) {
					AwsBedrockHandler.setEnv("AWS_PROFILE", this.config?.awsProfile)
				} else {
					delete process.env.AWS_PROFILE
					AwsBedrockHandler.setEnv("AWS_ACCESS_KEY_ID", this.config?.awsAccessKey)
					AwsBedrockHandler.setEnv("AWS_SECRET_ACCESS_KEY", this.config?.awsSecretKey)
					AwsBedrockHandler.setEnv("AWS_SESSION_TOKEN", this.config?.awsSessionToken)
				}
			},
			() => providerChain(),
		)
	...
	private getRegion(): string {
		return this.config?.awsRegion || AwsBedrockHandler.DEFAULT_REGION
	}
	...
	private async getBedrockClient(): Promise<BedrockRuntimeClient> {
		...
		if (this.config?.awsAuthentication === "apikey") {
			auth = {
				token: { token: this.config?.awsBedrockApiKey },
	...
	async getModelId(): Promise<string> {
		if (!this.config?.awsBedrockCustomSelected && this.config?.awsUseCrossRegionInference) {
			if (this.getModel().info.capabilities?.supportsGlobalEndpoint && this.config?.awsUseGlobalInference) {
	...
	private async *createDeepseekMessage(
		...
		const command = new InvokeModelWithResponseStreamCommand({
			...
			body: JSON.stringify({
				...
				max_tokens: model.info.capabilities?.maxTokens || 8000,
				temperature: 0,
			}),
		})
	...
	private getInferenceConfig(modelInfo: ModelInfo, modelType: "anthropic" | "nova", modelId?: string): any {
		...
		return {
			maxTokens: modelInfo.capabilities?.maxTokens || (modelType === "nova" ? 5000 : 8192),
			temperature: 0,
		}
	}
	...
	private async *createAnthropicMessage(
		...
		const messagesWithCache = this.config?.awsBedrockUsePromptCache
	...
	private async *createNovaMessage(
		...
		const messagesWithCache =
			this.config?.awsBedrockUsePromptCache && model.info.capabilities?.supportsPromptCache
				? this.applyCacheControlToMessages(formattedMessages, lastUserMsgIndex, secondLastMsgUserIndex)
				: formattedMessages

		// Prepare system message with caching support for Nova models that support it
		const enableCaching = this.config?.awsBedrockUsePromptCache && model.info.capabilities?.supportsPromptCache
	...
	private async *createOpenAIMessage(
		...
		const command = new ConverseCommand({
			...
			system: systemMessages,
			inferenceConfig: {
				maxTokens: model.info.capabilities?.maxTokens || 8192,
				temperature: 0,
			},
			...(toolConfig ? { toolConfig } : {}),
		})
