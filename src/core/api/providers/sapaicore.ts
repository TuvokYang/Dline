...
namespace Gemini {
	...
	export function prepareRequestPayload(
		...
		const payload = {
			...
			generationConfig: {
				maxOutputTokens: model.info.capabilities?.maxTokens,
				temperature: 0.0,
			},
		}
		...
		// if (thinkingBudget > 0 && model.info.capabilities?.thinking) {
		// 	;(payload as any).capabilities.thinking = {
...
export class SapAiCoreHandler implements ApiHandler {
	...
	private get config() {
		return this.ctx.profile.sapaicore
	}
	private get apiKey() {
	...
	private validateCredentials(): void {
		if (!this.config?.clientId || !this.config?.clientSecret || !this.config?.tokenUrl || !this.baseUrl) {
	...
	private async createAiCoreDestination(): Promise<HttpDestination> {
		try {
			...
			return {
				...destination,
				url: destination.url || this.baseUrl!,
			}
		} catch (error) {
	...
	private async authenticate(): Promise<Token> {
		...
		const tokenUrl = `${this.config?.tokenUrl?.replace(/\/+$/, "")}/oauth/token`
	...
	private async getAiCoreDeployments(): Promise<Deployment[]> {
		...
		const url = `${this.baseUrl}/v2/lm/deployments?$top=10000&$skip=0`
	...
	async *createMessage(systemPrompt: string, messages: ClineStorageMessage[]): ApiStream {
		if (this.config?.useOrchestrationMode) {
	...
	private async *createMessageWithDeployments(systemPrompt: string, messages: ClineStorageMessage[]): ApiStream {
		...
		const model = this.getModel()
		let deploymentId = (this.config as any)?.deploymentId
		...
		if (anthropicModels.includes(model.id)) {
			url = `${this.baseUrl}/v2/inference/deployments/${deploymentId}/invoke-with-response-stream`
			...
			if (
				...
				url = `${this.baseUrl}/v2/inference/deployments/${deploymentId}/converse-stream`
				...
				payload = {
					inferenceConfig: {
						maxTokens: model.info.capabilities?.maxTokens,
						temperature: 0.0,
					},
			...
			url = `${this.baseUrl}/v2/inference/deployments/${deploymentId}/chat/completions?api-version=2024-12-01-preview`
			payload = {
				stream: true,
			...
			url = `${this.baseUrl}/v2/inference/deployments/${deploymentId}/chat/completions`
			payload = {
				stream: true,
				messages: openAiMessages,
				temperature: 0.0,
			...
			url = `${this.baseUrl}/v2/inference/deployments/${deploymentId}/models/${model.id}:streamGenerateContent`
	...
	createUserReadableRequest(
		...
		return {
			...
			max_tokens: this.getModel().info.capabilities?.maxTokens,
