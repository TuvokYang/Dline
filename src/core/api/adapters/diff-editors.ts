...
function reconstructApplyPatchResult(
	...
	if (convertedToolName === "replace_in_file") {
		// Build the final_file_content block to preserve for downstream consumers
		// (e.g. model context, diff display) that expect this marker.
		const finalContentBlock = `<final_file_content path="${filePath}">\n${finalContent}\n</final_file_content>`
		...
		if (patchMatch) {
			...
			if (fileMatch) {
				const action = fileMatch[1]
				return (
					`[apply_patch for '${filePath}'] Result:\n` +
		...
		// Fallback for replace_in_file — still preserve final_file_content
		return (
			`[apply_patch for '${filePath}'] Result:\n` +
			`The content was successfully updated in ${filePath}.\n\n` +
			`${finalContentBlock}\n\n` +
...
function reconstructWriteToFileResult(block: any, originalToolName: string, originalInput: any): string | any[] {
	...
	const _finalContent = finalContentMatch[2]
