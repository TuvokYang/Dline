...
import { PlanActMode, UpdateSettingsRequestCli } from "@shared/proto/dline/state"
...
export async function updateSettingsCli(controller: Controller, request: UpdateSettingsRequestCli): Promise<Empty> {
	...
	if (request.settings) {
		// Proto-generated Settings may not include all runtime fields;
		...
		if (planModeReasoningEffort !== undefined) {
			...
			controller.stateManager.setGlobalState("planModeReasoningEffort" as any, converted)
		}

		if (actModeReasoningEffort !== undefined) {
			const converted = normalizeOpenaiReasoningEffort(actModeReasoningEffort)
			controller.stateManager.setGlobalState("actModeReasoningEffort" as any, converted)
		}
		...
		const didChangeProfile = typeof planModeProfile === "string" || typeof actModeProfile === "string"

		if (typeof planModeProfile === "string") {
			controller.stateManager.setGlobalState("planModeProfile", planModeProfile)
			if (controller.task) {
		...
		if (typeof actModeProfile === "string") {
			controller.stateManager.setGlobalState("actModeProfile", actModeProfile)
			if (controller.task) {
