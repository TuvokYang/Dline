...
async function fetchAndCacheModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	...
	// Find the first enabled groq profile with a valid apiKey
	let groqApiKey: string | undefined
	for (const p of findEnabledProfiles("groq")) {
			...
			if (response.data?.data) {
				...
				for (const rawModel of rawModels) {
					...
					const modelInfo: Partial<ModelInfo> = {
						capabilities: {
							maxTokens: rawModel.max_completion_tokens || staticModelInfo?.capabilities?.maxTokens || 8192,
						...
						pricing: {
							...
							outputPrice: staticModelInfo?.pricing?.outputPrice || 0,
