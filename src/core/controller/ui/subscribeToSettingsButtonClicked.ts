...
export async function subscribeToSettingsButtonClicked(
	...
	if (requestId)
		getRequestRegistry().registerRequest(requestId, cleanup, { type: "settingsButtonClicked_subscription" }, responseStream)
}

export async function sendSettingsButtonClickedEvent(controller: Controller): Promise<void> {
	...
	const promises = Array.from(set).map(async (rs) => {
		try {
			...
			Logger.error("Error sending settingsButtonClicked event:", e)
			set.delete(rs)
		}
	})
