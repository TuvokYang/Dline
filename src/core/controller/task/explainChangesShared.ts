...
const EXPLAINER_SYSTEM_PROMPT = `You are an AI coding assistant called Dline that will be explaining code changes to a developer. Your goal is to help the user understand what changed and why.
...
async function handleCommentReply(
	...
	// Disable thinking/reasoning for faster response.
	// Thinking budget fields are not part of ApiConfiguration; extend locally.
...
If the user asks you to make changes, fix something, or do any work that requires modifying code, let them know they can click the "Add to Dline Chat" button (the arrow icon in the top-right of the comment box) to send this conversation to the main Dline agent, which can then make the requested changes.
`
