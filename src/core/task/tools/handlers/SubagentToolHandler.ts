...
export class UseSubagentsToolHandler implements IFullyManagedTool {
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		const _config = uiHelpers.getConfig()
		const existingTs = block.ts
		if (shouldAutoApprove) {
			await uiHelpers.say("use_subagents", partialMessage, undefined, undefined, true, existingTs)
		} else {
			uiHelpers
				.ask("use_subagents", partialMessage, true, {
					existingTs,
				})
				.catch(() => {})
		}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!configuredSubagentName && prompts.length > MAX_SUBAGENT_PROMPTS) {
			...
			await config.callbacks.say(
				"use_subagents",
				JSON.stringify({
					prompts: prompts.slice(0, MAX_SUBAGENT_PROMPTS),
					error: "tooManyPrompts",
					message: `Too many subagent prompts provided (${prompts.length}). Maximum is ${MAX_SUBAGENT_PROMPTS}.`,
					count: prompts.length,
					max: MAX_SUBAGENT_PROMPTS,
				}),
				undefined,
				undefined,
				false,
		...
		if (didAutoApprove) {
			...
			showNotificationForApproval(
				prompts.length === 1
					? `Dline wants to use ${configuredSubagentName ? `the '${configuredSubagentName}' subagent` : "a subagent"}`
					: `Dline wants to use ${prompts.length} subagents`,
				config.autoApprovalSettings.enableNotifications,
			)
			const didApprove = await ToolResultUtils.askApprovalAndPushFeedback("use_subagents", approvalBody, config, block.ts)
			if (!didApprove) {
				telemetryService.captureToolUsage(
					config.ulid ?? "",
					this.name,
		...
		const emitStatus = async (status: ClineSaySubagentStatus["status"], partial: boolean) => {
			...
			await config.callbacks.say("subagent", JSON.stringify(payload), undefined, undefined, partial, block.ts)
		}
		...
		const execution = prompts.map((prompt, index) =>
			runners[index].run(prompt, async (update) => {
				...
				if (update.stats) {
					...
					if (update.stats.currency) {
						current.currency = update.stats.currency
					}
		...
		settled.forEach((result, index) => {
			if (result.status === "rejected") {
				...
				entries[index].error = (result.reason as Error)?.message || getPrompt("toolHandlers", "subagentExecutionFailed")
				return
			}
			...
			entries[index].currency = result.value.stats.currency || ""
