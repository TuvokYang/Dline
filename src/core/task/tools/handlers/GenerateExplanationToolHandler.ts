...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await uiHelpers.say("generate_explanation", messageText, undefined, undefined, true, block.ts)
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!title) {
			...
			return await config.callbacks.sayAndCreateMissingParamError(this.name, "title", undefined, block.ts)
		}

		if (!fromRef) {
			config.taskState.consecutiveMistakeCount++
			return await config.callbacks.sayAndCreateMissingParamError(this.name, "from_ref", undefined, block.ts)
		}
		...
		if (!apiConfiguration) {
			...
			return formatResponse.toolError(getPrompt("toolHandlers", "generateExplanationApiNotAvailable"))
		}
		...
		try {
			...
			if (diffSummary.files.length === 0) {
				// Finalize the loading partial before returning
				await config.callbacks.say(
					"generate_explanation",
			...
			if (config.taskState.abort) {
				...
				return formatResponse.toolResult(getPrompt("toolHandlers", "generateExplanationCancelled"))
			}
