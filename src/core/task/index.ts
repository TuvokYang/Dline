...
export class Task {
	...
	private async emitStateSnapshot(snapshot: TaskSnapshot): Promise<void> {
		try {
			// Write independent snapshot.json for direct human inspection
	...
	private getSnapshotActiveBlock(snapshot: TaskSnapshot): NonNullable<TaskSnapshot["approval"]>["blocks"][number] | undefined {
	...
	private findSnapshotAnchoredMessage(snapshot?: TaskSnapshot): ClineMessage | undefined {
		...
		const activeBlock = this.getSnapshotActiveBlock(snapshot)
		const anchorApiIndex = activeBlock?.apiIndex ?? snapshot.resume?.assistantApiIndex ?? snapshot.apiIndex
		const expectedAsk = activeBlock ? this.taskController.toolNameToAskType(activeBlock.name) : undefined
		const messages = this.messageStateHandler.clineMessages
	...
	async abortExecution() {
		try {
			...
			this.taskController.transition(TaskPhase.PAUSED, {
				apiIndex: this.messageStateHandler.apiConversationHistory.length - 1,
				onSnapshot: this.emitStateSnapshot.bind(this),
			})
