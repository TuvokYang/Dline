...
class ClineEndpoint {
	...
	private constructor() {
		...
		const _env =
			process?.env?.DLINE_ENVIRONMENT_OVERRIDE || process?.env?.DLINE_ENVIRONMENT || process?.env?.DLINE_ENVIRONMENT
	...
	public static isSelfHosted(): boolean {
		...
		const cfg = ClineEndpoint.config
		if (!cfg) return true
		return cfg.environment === Environment.selfHosted
	}
	 ...
	 * Returns undefined if not initialized (tests/module-load safe).
	 */
	public static get config(): EnvironmentConfig | undefined {
		if (!ClineEndpoint._initialized || !ClineEndpoint._instance) {
			return undefined
		}
	 ...
	 * Located at ~/.dline/endpoints.json
	 */
	private static getEndpointsFilePath(): string {
		return path.join(os.homedir(), ".dline", "endpoints.json")
	}
