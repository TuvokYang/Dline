			...
			vscode.window.withProgress(
				...
				async (progress) => {
					for (let i = 0; i < tasksCount; i++) {
						...
						await fs.writeFile(
							path.join(taskDir, GlobalFileNames.apiConversationHistory),
							JSON.stringify(
								[
									{
										role: "user",
						...
						await fs.writeFile(path.join(taskDir, GlobalFileNames.uiMessages), JSON.stringify(messages, null, 2))
