			...
			client[methodKey as keyof GrpcClientType<T>] = ((
				...
				return (async () => {
					try {
						...
						// biome-ignore lint/plugin: intentional — must not use Logger in gRPC path to avoid recursion
						console.error(
							`[Dline] Expected cancel function but got response object for streaming request: ${requestId}`,
						)
						return () => {}
					} catch (error) {
						// biome-ignore lint/plugin: intentional — must not use Logger in gRPC path to avoid recursion
						console.error(`[Dline] Error in streaming request:`, error)
						if (options.onError) {
			...
			client[methodKey as keyof GrpcClientType<T>] = ((request: any) => {
				return new Promise(async (resolve, reject) => {
					...
					try {
						...
						// biome-ignore lint/plugin: intentional — must not use Logger in gRPC path to avoid recursion
						console.debug(`[Dline] gRPC host ERR to ${service.fullName}.${methodKey} req:${requestId} err:${e}`)
						reject(e)
					}
				})
			}) as any
