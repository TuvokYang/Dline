...
e2e("Chat - can send messages and switch between modes", async ({ helper, sidebar }) => {
	...
	// Ensure What's New modal is dismissed before interacting with the UI
	await E2ETestHelper.dismissWhatsNewModal(sidebar)

	// Close current task if a task is active (X button in task header)
	const closeTaskButton = sidebar.getByLabel("New Task")
	if ((await closeTaskButton.count()) > 0) {
		await closeTaskButton.click()
	}
	await expect(sidebar.getByText("Recent")).toBeVisible()
	await expect(sidebar.getByText("Hello, Cline!").first()).toBeVisible()
