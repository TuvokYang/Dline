...
async function openModelConfiguration(page: Page, sidebar: Frame): Promise<Locator> {
	...
	const card = sidebar
		...
		.filter({ has: sidebar.locator(`input[value=${JSON.stringify(E2E_PROFILE_NAMES.mockAnthropic)}]`) })
	await expect(card).toHaveCount(1)
	const modelConfiguration = card.getByText("Model Configuration", { exact: true })
	if (!(await modelConfiguration.isVisible())) await card.getByRole("button").first().press("Enter")
