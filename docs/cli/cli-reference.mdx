...
|----------|-------------|
| `DLINE_DATA_DIR` | Custom configuration directory (replaces `~/.cline/data/`) |
| `DLINE_HUB_ADDRESS` | Override hub address (default: `127.0.0.1:25463`) |
| `DLINE_SESSION_BACKEND_MODE` | Force backend mode (`local`, `hub`, `remote`, `auto`) |
| `DLINE_SANDBOX_DATA_DIR` | Sandbox session storage directory |
| `DLINE_SANDBOX` | Enable sandbox mode |
| `DLINE_HOOKS_DIR` | Additional hooks directory |
| `DLINE_BUILD_ENV` | Set to `development` for debug features |
| `DLINE_DEBUG_PORT_BASE` | Base port for Node.js inspector |
| `DLINE_COMMAND_PERMISSIONS` | JSON policy restricting shell commands (see below) |
...
```bash
export DLINE_COMMAND_PERMISSIONS='{"allow": ["npm *", "git *"], "deny": ["rm -rf *", "sudo *"]}'
```
