...
| Event | Description | Key Attributes |
|-------|-------------|----------------|
| `dline.test.connection` | OTEL connection test event from "Test OTEL Connection" flow | success, exporter_type, endpoint |

### Example: dline.test.connection

```json
{
  "event": "dline.test.connection",
  ...
  "widgets": [
    {
      "definition": {
        ...
        "requests": [
          {
            "q": "sum:dline.task.completed{*}.as_count()",
            "display_type": "bars"
          }
        ],
