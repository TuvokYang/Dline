...
function generateClientInterfaceType(serviceName, serviceDefinition) {
	...
	const methods = Object.entries(serviceDefinition.service)
		.map(([methodName, methodDef]) => {
			const requestType = `proto.${getFqn(methodDef.requestType.type.name)}`
			const responseType = `proto.${getFqn(methodDef.responseType.type.name)}`
...
export class ${serviceName}ClientImpl 
	extends BaseGrpcClient<niceGrpc.dline.host.${serviceName}Client> 
	implements ${serviceName}ClientInterface {

	protected createClient(channel: Channel): niceGrpc.dline.host.${serviceName}Client {
		return createClient(niceGrpc.dline.host.${serviceName}Definition, channel)
	}
...
async function generateVscodeClientFile(hostServices) {
	...
	for (const [serviceName, serviceDefinition] of Object.entries(hostServices)) {
		...
		handlerMap.push(`	"dline.host.${serviceName}": {
